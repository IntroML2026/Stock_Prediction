...
MODEL_INFO = {
        ...
        "inputs": [{"name": k, "type": "number", "min": -1.0, "max": 1.0, "default": 0.0, "step": 0.01} for k in ['Sentiment ADBE','Sentiment MSFT','Sentiment JPM','Sentiment TARGET']]
}
...
def display_explanation(input_df, session, aws_bucket):
    ...
    st.pyplot(fig)
    # top feature 
    top_feature = pd.Series(shap_values[0].values, index=shap_values[0, :, 0].feature_names).abs().idxmax()
